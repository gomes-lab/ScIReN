...
# Data Path
#############################################################################
date_stamp = '20250630-003325_KAN_ONELAYER_GRIDMARGIN_COMPAS2_9917068_lr=1e-02_fold=0_seed=111'
...
var4nn_max_min = read.table(paste(data_dir_input, 'KAN_Propotional_Change/', 'grid_env_info_max_min.txt', sep = ''), sep = ',', header = FALSE, skip = 1)
...
input_matrix_color = c('#1b9e77', '#d95f02', '#7570b3', '#e7298a', '#66a61e', '#e6ab02', '#a6761d', '#666666', '#1b9e77', '#d95f02', '#7570b3')
process_name <- c(
...
for (i in 1:length(soc_category_names)) {
    temp_soc_baseline = read.table(paste(data_dir_input, 'KAN_Propotional_Change/', '/baseline', '/', soc_category_names[i], '.txt', sep = ''), sep = ',', header = FALSE)
...
for (var_idx in 1:length(var4nn)) {
    ...
    for (imatrix in 1:length(input_matrix)) {
      ...
      for (ichange in 1:length(percentage_change)) {
          ...
          if (input_matrix[imatrix] == 'bulk_I') {
              bulk_process_file = paste0(data_dir_input, 'KAN_Propotional_Change/', var_name_temp, '/prop_change_', sprintf("%.2f", percentage_change[ichange]), '_', 'para', '.txt')
              bulk_process_temp = read.table(bulk_process_file, sep = ',', header = FALSE)
              bulk_process_temp = as.data.frame(bulk_process_temp[ , 21])
          } else {
              bulk_process_file = paste0(data_dir_input, 'KAN_Propotional_Change/', var_name_temp, '/prop_change_', sprintf("%.2f", percentage_change[ichange]), '_', input_matrix[imatrix], '.txt')
              bulk_process_temp = read.table(bulk_process_file, sep = ',', header = FALSE)
          }
    ...
    plot_df <- as_tibble(bulk_process_prop_change) |>
            mutate(percentage = percentage_change * 100) |>
    ...
    env_info_update_value = read.table(paste(data_dir_input, 'KAN_Propotional_Change/', var_name_temp, '/grid_env_info_update.txt', sep = ''), sep = ',', header = FALSE)
    env_info_update_value = cbind(percentage_change * 100, env_info_update_value)
    ...
    p_bulk_process_updated <- p_bulk_process +
        scale_x_continuous(breaks = env_info_update_value$percentage_change,
...
for (var_idx in 1:length(var4nn)) {
    ...
    for (ichange in 1:length(percentage_change)) {
        # Load the data for the current variable and percentage change
        para_file = paste0(data_dir_input, 'KAN_Propotional_Change/', var_name_temp, '/prop_change_', sprintf("%.2f", percentage_change[ichange]), '_', 'para.txt')
    ...
    # Update the plot with the actual values of the environmental variable by applying the max and min values
    env_info_update_value = read.table(paste(data_dir_input, 'KAN_Propotional_Change/', var_name_temp, '/grid_env_info_update.txt', sep = ''), header = FALSE, sep = ',')
    env_info_update_value = cbind(percentage_change * 100, env_info_update_value)
    colnames(env_info_update_value) = c('percentage_change', 'env_info_update_value')
    env_info_update_value$env_info_actual_value = env_info_update_value$env_info_update_value * (var4nn_max_min[var_idx, 2] - var4nn_max_min[var_idx, 1]) + var4nn_max_min[var_idx, 1]
    # Replace the x axis with the updated values of the environmental variable
...
for (var_idx in 1:length(var4nn)) {
    ...
    for (icategory in 1:length(soc_category_names)) {
        ...
        for (ichange in 1:length(percentage_change)) {
            ...
            soc_file = paste0(data_dir_input, 'KAN_Propotional_Change/', var_name_temp, '/prop_change_', sprintf("%.2f", percentage_change[ichange]), '_', soc_category_names[icategory], '.txt')
            ...
            # Assign the data to the corresponding column in the array
            soc_prop_change_temp[ , ichange] = as.numeric(soc_temp[ , 1])
        }
    ...
    p_soc <- ggplot(plot_df, aes(x = percentage, y = prop_change, colour = soc_component)) +
       ...
       scale_y_continuous(breaks = seq(-100, y_limit, (y_limit - (-100))/10),
        ...
        coord_cartesian(ylim = c(-100, y_limit)) +
        labs(
          title = paste0(var_name[var_idx]),
          x     = "Change in environmental variable (%)",
