...
for ibootstrap = 1:10
    ...
    %%
    tri_matrix_summary_upper = nan(ValidGrid, 19);
    tri_matrix_summary_lower = nan(ValidGrid, 19);
    
    regress_summary_upper = nan(ValidGrid, 3);
    regress_summary_lower = nan(ValidGrid, 3);
    ...
    for iGrid = 1:length(GridInfo)
        ...
        [tri_diagnal_upper, tri_diagnal_lower] ...
            = fun_tri_matrix_output(Para, is_default, nbedrock, sand_vector, npp_mean, ...
        ...
        tri_matrix_summary_upper(iGrid, :) = tri_diagnal_upper;
        tri_matrix_summary_lower(iGrid, :) = tri_diagnal_lower;

        lm_results_upper = fitlm(log10(zsoi(tri_diagnal_upper > 10^(-20))), log10(tri_diagnal_upper(tri_diagnal_upper > 10^(-20))));
        lm_results_lower = fitlm(log10(zsoi(find(tri_diagnal_lower > 10^(-20))+1)), log10(tri_diagnal_lower(tri_diagnal_lower > 10^(-20))));
        

        regress_summary_upper(iGrid, :) = [lm_results_upper.Rsquared.Ordinary, lm_results_upper.Coefficients{1, 1}, lm_results_upper.Coefficients{2, 1}];
        regress_summary_lower(iGrid, :) = [lm_results_lower.Rsquared.Ordinary, lm_results_lower.Coefficients{1, 1}, lm_results_lower.Coefficients{2, 1}];
        
    end
    
    subplot(1, 3, 1)
    histogram(regress_summary_upper(:, 1), 30, 'FaceAlpha', 0.3, 'EdgeAlpha', 0.3)
    hold on
    histogram(regress_summary_lower(:, 1), 30, 'FaceAlpha', 0.3, 'EdgeAlpha', 0.3)
    xlabel('R2')
    subplot(1, 3, 2)
    histogram(regress_summary_upper(:, 2), 30, 'FaceAlpha', 0.3, 'EdgeAlpha', 0.3)
    hold on
    histogram(regress_summary_lower(:, 2), 30, 'FaceAlpha', 0.3, 'EdgeAlpha', 0.3)
    xlabel('Intercept')
    subplot(1, 3, 3)
    histogram(regress_summary_upper(:, 3), 30, 'FaceAlpha', 0.3, 'EdgeAlpha', 0.3)
    hold on
    histogram(regress_summary_lower(:, 3), 30, 'FaceAlpha', 0.3, 'EdgeAlpha', 0.3)
    xlabel('Slope')
    legend('Upper', 'Lower')

    set(gcf, 'Units', 'Inches', 'Position', [0, 0, 10, 5], 'PaperUnits', 'Inches', 'PaperSize', [10, 5])
    saveas(gcf, '/Users/ft254/GitHub/BINNS/figures/tri_matrix.pdf')
    close
end
