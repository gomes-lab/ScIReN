...
# print("Change x74 from 0.2 to 0.1")
print("Change f65 from 0.2 to 0.2")
# print("scale_factor for param_reg_loss is 12")
...
parser.add_argument("--best_model_val_metric", type=str, choices=["average_weighted_l1", "average_l1", "seperate_l1"], default="average_weighted_l1", help="Metric to use for selecting the best model. average_l1 is average of all L1 losses, seperate_l1 is L1 loss for soc and pom_maom.")
...
if args.vertical_mixing == 'original':
	para_names = ['diffus', 'cryo', 'q10', 'efolding', 'taucwd', 'taul1', 'taul2', 'tau4doc', 'tau4mic', 'tau4poc', 'tau4maom','fl1_MIC', 'fl2_POC', 'fMIC_MAOM', 'fMIC_POC', 'fDOC_MAOM', 'CUEl1', 'CUEl2', 'CUEDOC', 'w-scaling', 'beta'] # , 'fl1_POC'
else:
...
print("Parameter names: ", para_names)
print("Number of parameters: ", len(para_names))
...
def worker(rank, world_size, job_id, port):
	...
	for iepoch in range(start_epoch, num_epoch):
		...
		for batch_info in train_loader:
			...
			smooth_l1_loss, NSE_SOC, NSE_POM, NSE_MAOM, l1_loss_SOC, l1_loss_POM, l1_loss_MAOM, param_reg_loss = fun_loss(batch_y_hat, batch_y, batch_pred_para, args.param_reg_scale)
		...
		if rank == 0:
			...
			if val_loss_history_soc[iepoch, :] * args.lambdas[smooth_l1_loss_SOC_idx].item() + \
				...
				if args.model == "kan":
					...
					pruned_model = model_without_ddp.mlp.prune(node_th=0.01, edge_th=0.02)
	...
	if rank == 0:
		...
		with torch.no_grad():
			...
			best_guess_train_y_hat, best_guess_train_pred_para = best_guess_model(train_x.to(device), train_z.to(device), train_c.to(device),
																					whether_predict=0)
			train_smooth_l1_loss, train_NSE_SOC, train_NSE_POM, train_NSE_MAOM, train_l1_loss_SOC, train_l1_loss_POM, train_l1_loss_MAOM, train_param_reg_loss = fun_loss(best_guess_train_y_hat, train_y.to(device), best_guess_train_pred_para, args.param_reg_scale)
			...
			best_guess_val_y_hat, best_guess_val_pred_para = best_guess_model(val_x.to(device), val_z.to(device), val_c.to(device),
																				whether_predict=0)
			val_smooth_l1_loss, val_NSE_SOC, val_NSE_POM, val_NSE_MAOM, val_l1_loss_SOC, val_l1_loss_POM, val_l1_loss_MAOM, val_param_reg_loss = fun_loss(best_guess_val_y_hat, val_y.to(device), best_guess_val_pred_para, args.param_reg_scale)
			...
			if test_split_ratio != 0:
				...
				best_guess_test_y_hat, best_guess_test_pred_para = best_guess_model(test_x.to(device), test_z.to(device), test_c.to(device),
																					whether_predict=0)
				test_smooth_l1_loss, test_NSE_SOC, test_NSE_POM, test_NSE_MAOM, test_l1_loss_SOC, test_l1_loss_POM, test_l1_loss_MAOM, test_param_reg_loss = fun_loss(best_guess_test_y_hat, test_y.to(device), best_guess_test_pred_para, args.param_reg_scale)
				print(f'Test - smooth_l1_loss: {test_smooth_l1_loss.item():.2f}, NSE_SOC: {test_NSE_SOC.item():.2f}, NSE_POM: {test_NSE_POM.item():.2f}, NSE_MAOM: {test_NSE_MAOM.item():.2f}, l1_loss_SOC: {test_l1_loss_SOC.item():.2f}, l1_loss_POM: {test_l1_loss_POM.item():.2f}, l1_loss_MAOM: {test_l1_loss_MAOM.item():.2f}')
		...
		# Save pool data
		best_simu_soc_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		best_simu_POM_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		best_simu_MAOM_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		best_simu_DOC_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		best_simu_MIC_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		...
		# save pool data
		val_simu_soc_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		val_simu_POM_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		val_simu_MAOM_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		val_simu_DOC_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		val_simu_MIC_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		...
		# save pool data
		train_simu_soc_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		train_simu_POM_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		train_simu_MAOM_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		train_simu_DOC_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
		train_simu_MIC_pool = torch.tensor(np.ones((combined_profile_num, 20))*np.nan, dtype = torch.float32, device=device)
