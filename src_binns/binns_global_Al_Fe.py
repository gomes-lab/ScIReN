...
parser.add_argument("--parameter_weight", type=int, default=10, help="The regularization weight for the parameter constraint")
parser.add_argument("--parameter_scale", type=float, default=10, help="The scale for the parameter constraint")
...
# server path
job_submit_path = '/glade/u/home/haodixu/BINN/PBS_Submit/Global_Simulation/'
...
np.savetxt(data_dir_output + 'neural_network/' + job_id + '/obs_POM_matrix_1.csv', obs_POM_matrix, delimiter=",")
...
# print the first few rows of the grid env info index
print("Grid env info index", grid_env_info.index)

# Calculate the average values of the three layers for the grid env info
grid_env_info["Clay_Content_avg"] = (grid_env_info["Clay_Content_0cm"] + grid_env_info["Clay_Content_30cm"] + grid_env_info["Clay_Content_100cm"]) / 3
grid_env_info["Sand_Content_avg"] = (grid_env_info["Sand_Content_0cm"] + grid_env_info["Sand_Content_30cm"] + grid_env_info["Sand_Content_100cm"]) / 3
grid_env_info["Bulk_Density_avg"] = (grid_env_info["Bulk_Density_0cm"] + grid_env_info["Bulk_Density_30cm"] + grid_env_info["Bulk_Density_100cm"]) / 3
grid_env_info["SWC_v_Wilting_Point_avg"] = (grid_env_info["SWC_v_Wilting_Point_0cm"] + grid_env_info["SWC_v_Wilting_Point_30cm"] + grid_env_info["SWC_v_Wilting_Point_100cm"]) / 3
grid_env_info["pH_Water_avg"] = (grid_env_info["pH_Water_0cm"] + grid_env_info["pH_Water_30cm"] + grid_env_info["pH_Water_100cm"]) / 3
grid_env_info["CEC_avg"] = (grid_env_info["CEC_0cm"] + grid_env_info["CEC_30cm"] + grid_env_info["CEC_100cm"]) / 3
...
# grid_env_info = grid_env_info_US
# Check the max value of categorical variables, if it is larger than the number of categories, then remove the row
for group in categorical_vars:
	mask = grid_env_info[group].apply(lambda x: (x > np.max(env_info[group])).any(), axis=1)
	indices_to_remove = grid_env_info[mask].index
	grid_env_info = grid_env_info.drop(indices_to_remove)
	print("Shape of grid env info after removing rows with categorical values larger than the number of categories in category {}: ".format(group), grid_env_info.shape)
grid_env_info_num = grid_env_info.shape[0]
print("Shape of grid env info after selecting US", grid_env_info.shape)
...
for irow in np.arange(0, grid_env_info_num):
	lat_loc = np.where(abs(grid_env_info.iloc[irow, :]["original_lat"] - lat_grid) == min(abs(grid_env_info.iloc[irow, :]["original_lat"] - lat_grid)))[0][0]
	lon_loc = np.where(abs(grid_env_info.iloc[irow, :]["original_lon"] - lon_grid) == min(abs(grid_env_info.iloc[irow, :]["original_lon"] - lon_grid)))[0][0]
...
predict_data_x[:, 0:len(var4nn), 0, 0] = np.array(grid_env_info.loc[:, var4nn])
...
predict_data_profile_id = np.ones((grid_env_info_num))*np.nan
predict_data_profile_id = grid_env_info.index
...
predict_data_profile_id = predict_data_profile_id[valid_profile_loc]

print("Shape of predict data x", predict_data_x.shape)
print("Shape of predict data profile id", predict_data_profile_id.shape)
...
def binns_loss(y_pred, y_true, pred_para):
	...
	target_value = 0.5
	scale_factor = args.parameter_scale
	...
	regularization_weight = args.parameter_weight
...
class nn_model(nn.Module):
	...
	def forward(self, input_var, wosis_depth, whether_predict):
		...
		# 	# 	print(f"Variable index: {idx}")
		# 	# 	print(f"Predictor values: {predictor[:, idx]}")
		# 	# 	print(f"Embedding layer size: {embedding_layer.num_embeddings}")
		# 	# 	print("End of embedding layer")
		# 	## embedding layer ##
...
def worker(rank, world_size):
	...
	if args.whether_resume == 0:
		...
		val_NSE_history_POM = np.ones((num_epoch, 1))*np.nan
		val_NSE_history_MAOM = np.ones((num_epoch, 1))*np.nan
		...
		val_NSE_history_soc = checkpoint_worker['val_NSE_history_soc']
		val_NSE_history_POM = checkpoint_worker['val_NSE_history_POM']
		val_NSE_history_MAOM = checkpoint_worker['val_NSE_history_MAOM']
		best_model_epoch = checkpoint_worker['best_model_epoch']

		# Early stopping parameters
		best_val_loss = float('inf')  #checkpoint_worker['best_val_loss']
		best_val_NSE_soc = checkpoint_worker['best_val_NSE_soc']
		best_val_NSE_POM = checkpoint_worker['best_val_NSE_POM']
		best_val_NSE_MAOM = checkpoint_worker['best_val_NSE_MAOM'] 
		patience = args.patience
	...
	for iepoch in range(start_epoch, num_epoch):
		...
		val_pred_soc = torch.tensor(np.ones((combined_profile_num, 200))*np.nan, dtype = torch.float32, device=device)
		...
		test_pred_soc = torch.tensor(np.ones((combined_profile_num, 200))*np.nan, dtype = torch.float32, device=device)
		test_pred_para = torch.tensor(np.ones((combined_profile_num, len(para_names)))*np.nan, dtype = torch.float32, device=device)
		...
		NSE_POM_record_train = list()
		NSE_MAOM_record_train = list()
		ibatch = 0
		...
		for batch_info in train_loader:
			...
			total_loss, train_NSE_soc, train_NSE_POM, train_NSE_MAOM, smooth_l1_loss, smooth_l1_loss_POM, smooth_l1_loss_MAOM = fun_loss(batch_y_hat, batch_y, batch_pred_para)
			...
			l1_loss_POM_record_train.append(smooth_l1_loss_POM.item())
			l1_loss_MAOM_record_train.append(smooth_l1_loss_MAOM.item())
			NSE_soc_record_train.append(train_NSE_soc.item())
			NSE_POM_record_train.append(train_NSE_POM.item())
			NSE_MAOM_record_train.append(train_NSE_MAOM.item())
		...
		NSE_POM_record_val = list()
		NSE_MAOM_record_val = list()
		ibatch = 0
		model.eval()
		for batch_info in val_loader:
			...
			# 2 compute the objective function
			obj, val_NSE_soc, val_NSE_POM, val_NSE_MAOM, val_l1_loss, val_l1_loss_POM, val_l1_loss_MAOM = fun_loss(batch_y_hat, batch_y, batch_pred_para)
			...
			l1_loss_POM_record_val.append(val_l1_loss_POM.item())
			l1_loss_MAOM_record_val.append(val_l1_loss_MAOM.item())
			NSE_soc_record_val.append(val_NSE_soc.item())
			NSE_POM_record_val.append(val_NSE_POM.item())
			NSE_MAOM_record_val.append(val_NSE_MAOM.item())
		...
		dist.all_gather(all_train_NSE_soc, torch.tensor(NSE_soc_record_train, device=device).mean())
		dist.all_gather(all_train_NSE_POM, torch.tensor(NSE_POM_record_train, device=device).mean())
		dist.all_gather(all_train_NSE_MAOM, torch.tensor(NSE_MAOM_record_train, device=device).mean())
		...
		dist.all_gather(all_val_NSE_soc, torch.tensor(NSE_soc_record_val, device=device).mean())
		dist.all_gather(all_val_NSE_POM, torch.tensor(NSE_POM_record_val, device=device).mean())
		dist.all_gather(all_val_NSE_MAOM, torch.tensor(NSE_MAOM_record_val, device=device).mean())
		...
		val_NSE_history_soc[iepoch, :] = torch.stack(all_val_NSE_soc).mean().detach().cpu().numpy()
		val_NSE_history_POM[iepoch, :] = torch.stack(all_val_NSE_POM).mean().detach().cpu().numpy()
		val_NSE_history_MAOM[iepoch, :] = torch.stack(all_val_NSE_MAOM).mean().detach().cpu().numpy()
		...
		if rank == 3:
			...
			print(f'Epoch {iepoch}, train SOC NSE: {torch.stack(all_train_NSE_soc).mean():.2f}, train POM NSE: {torch.stack(all_train_NSE_POM).mean():.2f}, train MAOM NSE: {torch.stack(all_train_NSE_MAOM).mean():.2f}')
			print(f'Epoch {iepoch}, val SOC NSE: {torch.stack(all_val_NSE_soc).mean():.2f}, val POM NSE: {torch.stack(all_val_NSE_POM).mean():.2f}, val MAOM NSE: {torch.stack(all_val_NSE_MAOM).mean():.2f}')
			print(f'Epoch {iepoch}, train time: {torch.stack(all_train_times).mean():.2f}, hist time: {torch.stack(all_hist_times).mean():.2f}')
		...
		# 		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/nn_train_pred_para_' + job_id + "_" + str(iepoch) + '.csv', train_pred_para.detach().numpy(), delimiter = ',')
			...
			print(f'Epoch {iepoch}, train loss: {torch.stack(all_total_train_losses).mean():.2f}, train L1 loss: {torch.stack(all_l1_train_losses).mean():.2f}, train POM L1 loss: {torch.stack(all_l1_POM_train_losses).mean():.2f}, train MAOM L1 loss: {torch.stack(all_l1_MAOM_train_losses).mean():.2f}, \
		 validation loss: {torch.stack(all_total_val_losses).mean():.2f}, validation L1 loss: {torch.stack(all_l1_val_losses).mean():.2f}, validation POM L1 loss: {torch.stack(all_l1_POM_val_losses).mean():.2f}, validation MAOM L1 loss: {torch.stack(all_l1_MAOM_val_losses).mean():.2f}, \
			...
			if iepoch == 0:
			...
			elif val_NSE_history_soc[iepoch, :] < best_val_NSE_soc and val_loss_history_POM[iepoch, :] < best_val_NSE_POM and val_loss_history_MAOM[iepoch, :] < best_val_NSE_MAOM:
				# best_simu_soc = middle_simu_soc
				# best_pred_para = middle_pred_para
				...
				checkpoint_best_model = {
					...
					'best_val_NSE_POM': best_val_NSE_POM,
					'best_val_NSE_MAOM': best_val_NSE_MAOM,
					...
					'val_NSE_history_POM': val_NSE_history_POM,
					'val_NSE_history_MAOM': val_NSE_history_MAOM,
				...
				if iepoch > 250:
					with torch.no_grad():
						temp_soc_simu, temp_pred_para = model(test_x.to(device), test_x.to(device), whether_predict=0)
						test_obj, test_NSE_soc, test_NSE_POM, test_NSE_MAOM, test_l1_loss, test_l1_loss_POM, test_l1_loss_MAOM = fun_loss(temp_soc_simu, test_y, temp_pred_para)
					# save validation parameters
					print(f'Best model update at Epoch {iepoch}, test loss: {test_obj.item():.2f}, test NSE SOC: {test_NSE_soc.item():.2f}, test NSE POM: {test_NSE_POM.item():.2f}, test NSE MAOM: {test_NSE_MAOM.item():.2f}')
					print(f'Best model update at Epoch {iepoch}, test L1 loss: {test_l1_loss.item():.2f}, test L1 POM loss: {test_l1_loss_POM.item():.2f}, test L1 MAOM loss: {test_l1_loss_MAOM.item():.2f}')

					test_pred_soc[test_profile_id, :] = temp_soc_simu.detach().cpu()
					test_pred_para[test_profile_id, :] = temp_pred_para.detach().cpu()
					# print("Ending time to predict parameters: {}".format(datetime.now()))
					# save data
					np.savetxt(data_dir_output + 'neural_network/' + job_id + '/model_training_history/nn_test_pred_soc_' + job_id + "_" + str(iepoch) + '.csv', test_pred_soc.detach().numpy(), delimiter = ',')
					np.savetxt(data_dir_output + 'neural_network/' + job_id + '/model_parameters/nn_test_pred_para_' + job_id + "_" + str(iepoch) + '.csv', test_pred_para.detach().numpy(), delimiter = ',')
			...
			with open(os.path.join(data_dir_output, "neural_network", job_id, avg_l1_loss_filename), "a") as f:
				f.write(f'{iepoch}, {torch.stack(all_l1_train_losses).mean():.6f}, {torch.stack(all_l1_POM_train_losses).mean():.6f}, {torch.stack(all_l1_MAOM_train_losses).mean():.6f}, \
					{torch.stack(all_l1_val_losses).mean():.6f}, {torch.stack(all_l1_POM_val_losses).mean():.6f}, {torch.stack(all_l1_MAOM_val_losses).mean():.6f}, \
					{torch.stack(all_train_times).mean():.2f}, {torch.stack(all_hist_times).mean():.2f}, {best_model_epoch.item()}\n')
			with open(os.path.join(data_dir_output, "neural_network", job_id, avg_NSE_filename), "a") as f:
				f.write(f'{iepoch}, {torch.stack(all_train_NSE_soc).mean():.6f}, {torch.stack(all_train_NSE_POM).mean():.6f}, {torch.stack(all_train_NSE_MAOM).mean():.6f}, {torch.stack(all_val_NSE_soc).mean():.6f}, {torch.stack(all_val_NSE_POM).mean():.6f}, {torch.stack(all_val_NSE_MAOM).mean():.6f}, {torch.stack(all_train_times).mean():.2f}, {torch.stack(all_hist_times).mean():.2f}, {best_model_epoch.item()}\n')
		...
		if val_NSE_history_soc[iepoch, :] <= best_val_NSE_soc and val_NSE_history_POM[iepoch, :] <= best_val_NSE_POM and val_NSE_history_MAOM[iepoch, :] <= best_val_NSE_MAOM:
			best_val_NSE_soc = val_NSE_history_soc[iepoch, :]
			best_val_NSE_POM = val_NSE_history_POM[iepoch, :]
			best_val_NSE_MAOM = val_NSE_history_MAOM[iepoch, :]
		...
		if time.time() - job_begin_time > 41400:
			if rank == 0:
				...
				with open(job_submit_path + 'Resume' + job_id + '.submit', 'w') as f:
					...
					f.write(f'python -u /glade/u/home/haodixu/BINN/Server_Script/POM_MAOM/binns_global.py --lr ' + str(args.lr) + ' --weight_decay ' + str(args.weight_decay) + ' --batch_size ' + str(args.batch_size) + \
						...
						' --parameter_weight ' + str(args.parameter_weight) + ' --parameter_scale ' + str(args.parameter_scale) + '/n')
	...
	if rank == 0:
		...
		with torch.no_grad():
			...
			if test_split_ratio != 0:
				...
				test_loss, test_NSE_soc, test_NSE_POM, test_NSE_MAOM, test_l1_loss, test_l1_POM_loss, test_l1_MAOM_loss = fun_loss(best_guess_test_y_hat, test_y.to(device), best_guess_test_pred_para)
		...
		results_summary_file = os.path.join(data_dir_output, "neural_network/results_summary_POM_MAOM_COMPAS.csv")
		if not os.path.isfile(results_summary_file):
			with open(results_summary_file, mode='w') as f:
				csv_writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
				csv_writer.writerow(['job_id', 'command', 'epoch', 'lr', 'weight_decay', 'seed', 'model_path', 'best_val_NSE_soc', 'best_val_NSE_POM', 'best_val_NSE_MAOM', 'best_val_loss',
						 'test_soc_NSE', 'test_soc_loss', 'test_POM_NSE', 'test_POM_loss', 'test_MAOM_NSE', 'test_MAOM_loss', 'Overall_loss', 'Paramter_Weight', 'Parameter_Scale'])
		...
		with open(results_summary_file, mode='a+') as f:
			csv_writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
			best_model_path = data_dir_output + 'neural_network/' + job_id + '/opt_nn_' + job_id  + '.pt'
			csv_writer.writerow([job_id, command_string, args.n_epochs, args.lr, args.weight_decay, args.seed, best_model_path, best_val_NSE_soc.item(), best_val_NSE_POM.item(), best_val_NSE_MAOM.item(), best_val_loss.item(),
					 test_NSE_soc.item(), test_l1_loss.item(), test_NSE_POM.item(), test_l1_POM_loss.item(), test_NSE_MAOM.item(), test_l1_MAOM_loss.item(), test_loss.item(), args.parameter_weight, args.parameter_scale])
