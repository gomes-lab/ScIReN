...
# This script is used to resume training from a checkpoint, also automatically resubmits the job after 11.5 hours
...
# Get previous job ID from the environment variable set by PBS
...
def worker(rank, world_size):
	...
	for iepoch in range(start_epoch, num_epoch):
		...
		# If runtimes are over 11.5 hours, save checkpoint and exit
			...
			if np.isnan(binn_obs_soc[i, :]).all() or np.isnan(best_simu_soc[i, :]).all():
				...
				for j in range(len(simu_soc)):
					if np.isnan(obs_soc[j]) or np.isnan(simu_soc[j]):
						...
						if j >= 25:
							print('outlier: ', test_profile_id_all[i], j, simu_soc[j], obs_soc[j])
							continue
			...
			if np.isnan(val_simu_soc[i, :]).all() or np.isnan(binn_obs_soc[i, :]).all():
				...
				for j in range(len(simu_soc)):
					if np.isnan(obs_soc[j]) or np.isnan(simu_soc[j]):
						...
						if j >= 25:
							print('outlier: ', val_profile_id_all[i], j, simu_soc[j], obs_soc[j])
							continue
						# Calculate the scaled difference
