...
def fun_model_simu(tensor_para, tensor_frocing_steady_state):
	...
	simu_ouput = torch.ones((profile_num, 20))*torch.nan
	# calculate soc solution for each profile
	for iprofile in torch.arange(0, profile_num):
...
def fun_matrix_clm5(para, frocing_steady_state):
	...
	days_per_year = 365
	secspday = 24*60*60
	days_per_month = torch.tensor([[31, 30, 31, 28, 31, 30, 31, 31, 30, 31, 30, 31]]).transpose(0, 1)
	days_per_season = torch.tensor([[92, 89, 92, 92]]).transpose(0, 1)
	...
	dz_node = zsoi - torch.cat((torch.tensor([0.0]), zsoi[:-1]), axis = 0)
	
	#---------------------------------------------------
	# observed soc info
	#---------------------------------------------------
	obs_soc = frocing_steady_state[0:500, 0, 0]
	obs_layer_num = len(torch.where(torch.isnan(obs_soc) == False)[0])
	...
	#---------------------------------------------------
	xit = torch.ones(n_soil_layer, timestep_num)*torch.nan 
	xiw = torch.ones(n_soil_layer, timestep_num)*torch.nan 
	...
	for itimestep in range(timestep_num):
		...
		catanf_30 = catanf(torch.tensor(30.0))
		normalization_tref = torch.tensor(15)
	...
	#---------------------------------------------------
	sand_vector = torch.nanmean(sand_vector_steady_state, axis = 1)
	# allocation matrix
	a_ma = a_matrix(fl1s1, fl2s1, fl3s2, fs1s2, fs1s3, fs2s1, fs2s3, fs3s1, fcwdl2, sand_vector)
	
	kk_ma_middle = torch.zeros([npool_vr, npool_vr, timestep_num])*torch.nan 
	tri_ma_middle = torch.zeros([npool_vr, npool_vr, timestep_num])*torch.nan 
	...
	m_to_cm = 100
	vertical_prof = torch.ones(n_soil_layer)*torch.nan 
	...
	#---------------------------------------------------
	matrix_in = torch.ones([npool_vr, 1])*torch.nan
	# total input amount
	...
	try:
		...
		cpool_steady_state = torch.ones([140, 1])*(-9999.) 
	# end try
...
def a_matrix(fl1s1, fl2s1, fl3s2, fs1s2, fs1s3, fs2s1, fs2s3, fs3s1, fcwdl2, sand_vector):
	...
	nspools_vr = npool_vr
	# a diagnal matrix
	a_ma_vr = torch.diag(-1*torch.ones(nspools_vr))
...
def kk_matrix(xit, xiw, xio, xin, efolding, tau4cwd, tau4l1, tau4l2, tau4l3, tau4s1, tau4s2, tau4s3):
	...
	kk_ma_vr = torch.zeros([npool_vr, npool_vr])
...
def tri_matrix(nbedrock, altmax, altmax_lastyear, som_diffus, som_adv_flux, cryoturb_diffusion_k):
	...
	tri_ma = torch.zeros([npool_vr, npool_vr])

	som_adv_coef = torch.zeros(nlevdecomp+1) # SOM advective flux (m/day)
	som_diffus_coef = torch.zeros(nlevdecomp+1) # SOM diffusivity due to bio/cryo-turbation (m2/day)
	diffus = torch.zeros(nlevdecomp+1) # diffusivity (m2/day)  (includes spinup correction, if any)
	adv_flux = torch.zeros(nlevdecomp+1) # advective flux (m/day)  (includes spinup correction, if any)

	a_tri_e = torch.zeros(nlevdecomp) # "a" vector for tridiagonal matrix
	b_tri_e = torch.zeros(nlevdecomp) # "b" vector for tridiagonal matrix
	c_tri_e = torch.zeros(nlevdecomp) # "c" vector for tridiagonal matrix
	r_tri_e = torch.zeros(nlevdecomp) #"r" vector for tridiagonal solution

	a_tri_dz = torch.zeros(nlevdecomp) # "a" vector for tridiagonal matrix with considering the depth
	b_tri_dz = torch.zeros(nlevdecomp) # "b" vector for tridiagonal matrix with considering the depth
	c_tri_dz = torch.zeros(nlevdecomp) # "c" vector for tridiagonal matrix with considering the depth

	d_p1_zp1 = torch.zeros(nlevdecomp+1) # diffusivity/delta_z for next j
	# (set to zero for no diffusion)
	d_m1_zm1 = torch.zeros(nlevdecomp+1) # diffusivity/delta_z for previous j
	# (set to zero for no diffusion)
	f_p1 = torch.zeros(nlevdecomp+1) # water flux for next j
	f_m1 = torch.zeros(nlevdecomp+1) # water flux for previous j
	pe_p1 = torch.zeros(nlevdecomp+1) # Peclet # for next j
	pe_m1 = torch.zeros(nlevdecomp+1) # Peclet # for previous j
	
	w_p1 = torch.zeros(nlevdecomp+1)
	w_m1 = torch.zeros(nlevdecomp+1)
...
def catanf(t1):
	catanf_results = 11.75 +(29.7 / torch.pi) * torch.arctan( torch.pi * 0.031  * ( t1 - 15.4 ))
