...
def fun_model_simu(tensor_para, tensor_frocing_steady_state, tensor_obs_layer_depth):
	...
	for iprofile in range(0, profile_num):
		...
		if torch.isnan(torch.sum(profile_para)) == False and \
			...
			# print(profile_para)
			# model simulation
			profile_simu_soc, profile_simu_POM, profile_simu_MAOM = matrix_fun_COMPAS(profile_para, profile_force_steady_state)
...
def fun_model_sensitivity(tensor_para, tensor_frocing_steady_state):
	...
	simu_ouput_soc = (torch.ones((profile_num, 20))*np.nan).to(device)
	simu_output_POM = (torch.ones((profile_num, 20))*np.nan).to(device)
	simu_output_MAOM = (torch.ones((profile_num, 20))*np.nan).to(device)
	...
	for iprofile in range(0, profile_num):
		...
		if torch.isnan(torch.sum(profile_para)) == False and \
			...
			# save simulation results
			simu_ouput_soc[iprofile, 0:20] = profile_simu_soc
			simu_output_POM[iprofile, 0:20] = profile_simu_POM
			simu_output_MAOM[iprofile, 0:20] = profile_simu_MAOM

	#end for iprofile
	return simu_ouput_soc, simu_output_POM, simu_output_MAOM
...
def matrix_fun_COMPAS(tensor_para, tensor_forcing_steady_state):
	...
	tau4l1 = scale(para[5], 0.0001, 0.11)
	tau4l2 = scale(para[6], 0.1, 0.3)
	tau4doc = scale(para[7], 0.0001, 1)
	tau4mic = scale(para[8], 0.0001, 1)
	tau4poc = scale(para[9], 1, 30)
	tau4maom = scale(para[10], 80, 200)

	f42 = scale(para[11], 0.1, 0.8)
	x52 = scale(para[12], 0.05, 0.6)
	x53 = scale(para[13], 0.0001, 0.4)
	f73 = scale(para[14], 0.0001, 0.15)
	f45 = scale(para[15], 0.3, 0.8)
	f65 = scale(para[16], 0.0001, 0.4)
	f46 = scale(para[17], 0.5, 0.8)

	e52 = scale(para[18], 0.0001, 0.7)
	e53 = scale(para[19], 0.0001, 0.4)
	e54 = scale(para[20], 0.0001, 0.5)
	w_scaling = scale(para[21], 0.0001, 5)
	beta = scale(para[22], 0.5, 0.9999)
	...
	a_ma = a_matrix(f42, x52, x53, f73, f45, f65, f46, e52, e53, e54, sand_vector)
	...
	soc_layer = torch.cat((cpool_steady_state[100:120, :], cpool_steady_state[120:140, :]), dim = 1)
	...
	# outcome = soc_layer
	return soc_layer, POM_layer, MAOM_layer
...
def a_matrix(f42, x52, x53, f73, f45, f65, f46, E52, E53, E54, sand_vector):
    n_soil_layer = 20
    npool = 7
    npool_vr = 140
    ...
    transfer_fraction = [fcwdl2, f42, f52, f62, f53, f63, f73, f54, f45, f65, f75, f46, f76, f47]
    ...
    for j in range(n_soil_layer):
        ...
        a_ma_vr[(6 - 1) * n_soil_layer + j, (2 - 1) * n_soil_layer + j] = transfer_fraction[3]
        a_ma_vr[(5 - 1) * n_soil_layer + j, (3 - 1) * n_soil_layer + j] = transfer_fraction[4]
        a_ma_vr[(6 - 1) * n_soil_layer + j, (3 - 1) * n_soil_layer + j] = transfer_fraction[5]
        a_ma_vr[(7 - 1) * n_soil_layer + j, (3 - 1) * n_soil_layer + j] = transfer_fraction[6]
        a_ma_vr[(5 - 1) * n_soil_layer + j, (4 - 1) * n_soil_layer + j] = transfer_fraction[7]
        a_ma_vr[(4 - 1) * n_soil_layer + j, (5 - 1) * n_soil_layer + j] = transfer_fraction[8]
        a_ma_vr[(6 - 1) * n_soil_layer + j, (5 - 1) * n_soil_layer + j] = transfer_fraction[9]
        a_ma_vr[(7 - 1) * n_soil_layer + j, (5 - 1) * n_soil_layer + j] = transfer_fraction[10]
        a_ma_vr[(4 - 1) * n_soil_layer + j, (6 - 1) * n_soil_layer + j] = transfer_fraction[11]
        a_ma_vr[(7 - 1) * n_soil_layer + j, (6 - 1) * n_soil_layer + j] = transfer_fraction[12]
        a_ma_vr[(4 - 1) * n_soil_layer + j, (7 - 1) * n_soil_layer + j] = transfer_fraction[13]
...
def kk_matrix(xit, xiw, xio, xin, efolding, tau4cwd, tau4l1, tau4l2, tau4doc, tau4mic, tau4poc, tau4maom):
	...
	diagonal_vector = torch.concatenate([kcwd * xi_tw * depth_scalar,
										 ...
										 kdoc * xi_tw * depth_scalar * n_scalar,
