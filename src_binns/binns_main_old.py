...
#---------------------------------------------------
obs_soc_matrix = np.ones([len(profile_collection), 500])*np.nan
obs_depth_matrix = np.ones([len(profile_collection), 500])*np.nan
...
current_data_y = np.ones((len(profile_collection), 500, 12, 14))*np.nan

current_data_y[:, 0:500, 0, 0] = obs_soc_matrix
current_data_y[:, 0:500, 0, 1] = obs_depth_matrix

current_data_y[:, 0:12, 0, 2] = model_force_input_vector_cwd
current_data_y[:, 0:12, 0, 3] = model_force_input_vector_litter1
current_data_y[:, 0:12, 0, 4] = model_force_input_vector_litter2
current_data_y[:, 0:12, 0, 5] = model_force_input_vector_litter3
current_data_y[:, 0:12, 0, 6] = model_force_altmax_lastyear_profile
current_data_y[:, 0:12, 0, 7] = model_force_altmax_current_profile
current_data_y[:, 0:12, 0, 8] = model_force_nbedrock

current_data_y[:, 0:20, 0:12, 9] = model_force_xio
current_data_y[:, 0:20, 0:12, 10] = model_force_xin
current_data_y[:, 0:20, 0:12, 11] = model_force_sand_vector
current_data_y[:, 0:20, 0:12, 12] = model_force_soil_temp_profile
current_data_y[:, 0:20, 0:12, 13] = model_force_soil_water_profile
...
train_loc = np.arange(0, round(current_data_y.shape[0]*0.8))
# split into input and outputs
train_x = current_data_x[train_loc, :]
train_y = current_data_y[train_loc, :, :, :]

# test
pred_para = np.ones((len(train_loc), len(para_names)))*0.3
soc_simu = fun_model_simu(tf.convert_to_tensor(pred_para[0:32, :]), tf.convert_to_tensor(train_y[0:32, :, :, :]))
soc_true =  tf.convert_to_tensor(train_y[0:32, :, 0, 0])


#---------------------------------------------------
# define the loss function                          
#---------------------------------------------------
def binns_loss(y_true, y_pred):
	soc_simu = fun_model_simu(y_pred, y_true)
	soc_true = y_true[:, :, 0, 0]
	
	soc_simu_vector = K.reshape(soc_simu, [-1, 1]).numpy()[:, 0]
	soc_true_vector = K.reshape(soc_true, [-1, 1]).numpy()[:, 0]

	valid_loc = np.where(np.isnan(soc_simu_vector+soc_true_vector) == False)
	soc_simu_vector = tf.convert_to_tensor(soc_simu_vector[valid_loc], dtype = tf.float32)
	soc_true_vector = tf.convert_to_tensor(soc_true_vector[valid_loc], dtype = tf.float32)

	# modeling_efficiency = 1 - np.sum((soc_simu_vector - soc_true_vector)**2)/np.sum((np.mean(soc_true_vector) - soc_true_vector)**2)
	
	# print(soc_simu_vector+soc_true_vector)
	# 
	modeling_inefficiency = K.sum(K.square(soc_simu_vector - soc_true_vector))/K.sum(K.square(K.mean(soc_true_vector) - soc_true_vector))
	# print(tf.convert_to_tensor((1 - modeling_efficiency)*100))
...
model.add(Dense(len(para_names), activation='relu'))

model.compile(loss = binns_loss, optimizer = nn_optimizer, metrics = None, run_eagerly = True)
