...
from multiprocessing import Process  # @joshuafan: move random seed below
...
for group in categorical_vars:
	...
	# @joshuafan - can try increasing embedding_dim, e.g. to 15
	emb = nn.Embedding(num_embeddings=n_categories, embedding_dim=15).to(device)
	for var in group:
...
if test_split_ratio == 0:
	...
	train_y = torch.tensor(current_data_y[train_loc, :], dtype = torch.float32, device=device)
	val_y = torch.tensor(current_data_y[val_loc, :], dtype = torch.float32, device=device)

	train_z = torch.tensor(current_data_z[train_loc, :], dtype = torch.float32, device=device)
	val_z = torch.tensor(current_data_z[val_loc, :], dtype = torch.float32, device=device)

	train_x = torch.tensor(current_data_x[train_loc, :, :, :], dtype = torch.float32, device=device)
	val_x = torch.tensor(current_data_x[val_loc, :, :, :], dtype = torch.float32, device=device)

	train_profile_id = torch.tensor(current_data_profile_id[train_loc], dtype = torch.long, device=device)
	val_profile_id = torch.tensor(current_data_profile_id[val_loc], dtype = torch.long, device=device)
else:
	...
	train_y = torch.tensor(current_data_y[train_loc, :], dtype=torch.float32, device=device)
	val_y = torch.tensor(current_data_y[val_loc, :], dtype=torch.float32, device=device)
	test_y = torch.tensor(current_data_y[test_loc, :], dtype=torch.float32, device=device)

	train_z = torch.tensor(current_data_z[train_loc, :], dtype=torch.float32, device=device)
	val_z = torch.tensor(current_data_z[val_loc, :], dtype=torch.float32, device=device)
	test_z = torch.tensor(current_data_z[test_loc, :], dtype=torch.float32, device=device)

	train_x = torch.tensor(current_data_x[train_loc, :, :, :], dtype=torch.float32, device=device)
	val_x = torch.tensor(current_data_x[val_loc, :, :, :], dtype=torch.float32, device=device)
	test_x = torch.tensor(current_data_x[test_loc, :, :, :], dtype=torch.float32, device=device)
...
# Loss and optimizer
optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)  # lr=0.0001, weight_decay=0.01)
fun_loss = binns_loss
...
# Single-rpcoess: no distributed sampler
train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=args.batch_size)

# training and validation loop
# record the loss history
train_loss_history = np.ones((args.n_epochs, 1))*np.nan
val_loss_history = np.ones((args.n_epochs, 1))*np.nan
...
for iepoch in range(args.n_epochs):
	...
	train_loss_history[iepoch, :] = torch.tensor(loss_record_train).mean()
	val_loss_history[iepoch, :] = torch.tensor(loss_record_val).mean()

	# Record loss to Tensorboard, stdout, file
	writer.add_scalars('loss', {'training': torch.tensor(loss_record_train).mean(), 'validation': torch.tensor(loss_record_val).mean()}, iepoch+1)
	print(f'Epoch {iepoch + 1}, train loss: {torch.tensor(loss_record_train).mean():.2f}, validation loss: {torch.tensor(loss_record_val).mean():.2f}, time: {train_time:.2f}')
	with open(os.path.join(data_dir_output, "neural_network", job_id, avg_loss_filename), "a") as f:
		f.write(f'{iepoch + 1}, {torch.tensor(loss_record_train).mean():.2f}, {torch.tensor(loss_record_val).mean():.2f}, {train_time:.2f}\n')
	...
	if val_loss_history[iepoch, :] <= best_val_loss:
		...
		# @joshuafan: save best model path
	...
	else:  	# end if iepoch == 0:
		epochs_without_improvement += 1

	# Early stopping condition
	if epochs_without_improvement == args.patience:
		print("Early stopping due to no improvement after {} epochs.".format(args.patience))
...
# best_guess_model = DDP(best_guess_model)  # @joshuafan commented out
...
with torch.no_grad():
	...
	if test_split_ratio != 0:
		best_guess_test_y_hat, best_guess_test_pred_para = best_guess_model(test_x.to(device), test_z.to(device))
		test_loss = fun_loss(best_guess_test_y_hat, test_y.to(device))
...
if not os.path.isfile(results_summary_file):
	with open(results_summary_file, mode='w') as f:
		...
		csv_writer.writerow(['job_id', 'git_commit', 'command', 'lr', 'weight_decay', 'seed', 'model_path', 'val_loss'])
...
with open(results_summary_file, mode='a+') as f:
	csv_writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	csv_writer.writerow([job_id, git_commit, command_string, args.lr, args.weight_decay, args.seed, best_model_path, best_val_loss])
