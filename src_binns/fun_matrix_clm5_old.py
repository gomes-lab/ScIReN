import numpy as np
# from scipy.sparse.linalg import spsolve as matrix_solver
...
def fun_model_simu(tensor_para, tensor_frocing_steady_state):
	...
	para = tensor_para.numpy()
	frocing_steady_state = tensor_frocing_steady_state.numpy()
	# final ouputs of simulation
	profile_num = para.shape[0]
	simu_ouput = np.ones((profile_num, 500))*np.nan
	# calculate soc solution for each profile
	for iprofile in np.arange(0, profile_num):
		profile_para = para[iprofile, :]
		profile_force_steady_state = frocing_steady_state[iprofile, :, :, :]
		
		if np.isnan(np.sum(profile_para)) == False and \
			np.isnan(np.sum(profile_force_steady_state[0:12, 0, 2:9])) == False and \
			np.isnan(np.sum(profile_force_steady_state[0:20, 0:12, 9:14])) == False:
			
			profile_para[profile_para <= 0] = 0.0001
			# print(profile_para)
		...
		# end if 
	#end for iprofile
	simu_ouput = tf.convert_to_tensor(simu_ouput)
	return simu_ouput
...
def fun_matrix_clm5(para, frocing_steady_state):
	...
	days_per_year = 365
	secspday = 24*60*60
	days_per_month = np.array([[31, 30, 31, 28, 31, 30, 31, 31, 30, 31, 30, 31]]).transpose()
	days_per_season = np.array([[92, 89, 92, 92]]).transpose()
	...
	# observed soc info
	#---------------------------------------------------
	obs_soc = frocing_steady_state[0:500, 0, 0]
	obs_depth = frocing_steady_state[0:500:, 0, 1]
	obs_layer_num = len(np.where(np.isnan(obs_soc) == False)[0])
	obs_soc = obs_soc[0:obs_layer_num]
	obs_depth = obs_depth[0:obs_layer_num]
	#---------------------------------------------------
	# steady state forcing
	#---------------------------------------------------
	input_vector_cwd_steady_state = frocing_steady_state[0:12, 0, 2]
	input_vector_litter1_steady_state = frocing_steady_state[0:12, 0, 3]
	input_vector_litter2_steady_state = frocing_steady_state[0:12, 0, 4]
	input_vector_litter3_steady_state = frocing_steady_state[0:12, 0, 5]
	altmax_lastyear_profile_steady_state = frocing_steady_state[0:12, 0, 6]
	altmax_current_profile_steady_state = frocing_steady_state[0:12, 0, 7]
	nbedrock_steady_state = frocing_steady_state[0:12, 0, 8]
	
	xio_steady_state = frocing_steady_state[0:20, 0:12, 9]
	xin_steady_state = frocing_steady_state[0:20, 0:12, 10]
	sand_vector_steady_state = frocing_steady_state[0:20, 0:12, 11]
	soil_temp_profile_steady_state = frocing_steady_state[0:20, 0:12, 12]
	soil_water_profile_steady_state = frocing_steady_state[0:20, 0:12, 13]
	...
	w_scaling = para[20]*(5 - 0) + 0
	# beta to describe the shape of vertical profile
	# beta = 0.95
	beta = para[18]*(0.9999 - 0.5) + 0.5
	...
	#---------------------------------------------------
	xit = np.ones([n_soil_layer, timestep_num])*np.nan 
	xiw = np.ones([n_soil_layer, timestep_num])*np.nan 
	...
	#---------------------------------------------------
	sand_vector = np.nanmean(sand_vector_steady_state, axis = 1)
	# allocation matrix
	a_ma = a_matrix(fl1s1, fl2s1, fl3s2, fs1s2, fs1s3, fs2s1, fs2s3, fs3s1, fcwdl2, sand_vector)
	
	kk_ma_middle = np.zeros([npool_vr, npool_vr, timestep_num])*np.nan 
	tri_ma_middle = np.zeros([npool_vr, npool_vr, timestep_num])*np.nan 
	...
	for itimestep in range(timestep_num):
		...
		# K matrix
		kk_ma_middle[:, :, itimestep] = kk_matrix(timesteply_xit, timesteply_xiw, timesteply_xio, timesteply_xin, efolding, tau4cwd, tau4l1, tau4l2, tau4l3, tau4s1, tau4s2, tau4s3)
		# tri matrix
		timesteply_nbedrock = int(nbedrock_steady_state[itimestep])
	...
	# end for itimestep
	tri_ma = np.mean(tri_ma_middle, axis = 2)
	kk_ma = np.mean(kk_ma_middle, axis = 2)
	...
	m_to_cm = 100
	vertical_prof = np.ones(n_soil_layer)*np.nan 
	if np.mean(altmax_lastyear_profile_steady_state) > 0:
	...
	#---------------------------------------------------
	matrix_in = np.ones([npool_vr, 1])*np.nan
	# total input amount
	input_tot_cwd = np.nansum(input_vector_cwd_steady_state)/days_per_year # (gc/m2/day)
	input_tot_litter1 = np.nansum(input_vector_litter1_steady_state)/days_per_year # (gc/m2/day)
	input_tot_litter2 = np.nansum(input_vector_litter2_steady_state)/days_per_year # (gc/m2/day)
	input_tot_litter3 = np.nansum(input_vector_litter3_steady_state)/days_per_year # (gc/m2/day)
	...
	try:
		cpool_steady_state = matrix_solver((np.matmul(a_ma, kk_ma)-tri_ma), (-matrix_in))
		cpool_steady_state[cpool_steady_state < 0] = 0
		
		soc_layer = np.array([cpool_steady_state[80:100, 0], cpool_steady_state[100:120, 0], cpool_steady_state[120:140, 0]]).transpose()
		soc_layer = np.nansum(soc_layer, axis = 1) # unit gC/m3
		
		simu_soc_interp = pchip_interpolate(zsoi[0:n_soil_layer], soc_layer, obs_depth)
	except:
		simu_soc_interp = obs_soc*np.nan 
	# end try
	
	outcome = simu_soc_interp
	return outcome
...
def a_matrix(fl1s1, fl2s1, fl3s2, fs1s2, fs1s3, fs2s1, fs2s3, fs3s1, fcwdl2, sand_vector):
	...
	nspools_vr = npool_vr
	# a diagnal matrix
	a_ma_vr = np.diag(-1*np.ones(nspools_vr))
...
def kk_matrix(xit, xiw, xio, xin, efolding, tau4cwd, tau4l1, tau4l2, tau4l3, tau4s1, tau4s2, tau4s3):
	...
	depth_scalar = np.exp(-zsoi/decomp_depth_efolding)
	xi_tw = t_scalar*w_scalar*o_scalar
	
	kk_ma_vr = np.zeros([npool_vr, npool_vr])
...
def tri_matrix(nbedrock, altmax, altmax_lastyear, som_diffus, som_adv_flux, cryoturb_diffusion_k):
	...
	tri_ma = np.zeros([npool_vr, npool_vr])

	som_adv_coef = np.zeros(nlevdecomp+1) # SOM advective flux (m/day)
	som_diffus_coef = np.zeros(nlevdecomp+1) # SOM diffusivity due to bio/cryo-turbation (m2/day)
	diffus = np.zeros(nlevdecomp+1) # diffusivity (m2/day)  (includes spinup correction, if any)
	adv_flux = np.zeros(nlevdecomp+1) # advective flux (m/day)  (includes spinup correction, if any)

	a_tri_e = np.zeros(nlevdecomp) # "a" vector for tridiagonal matrix
	b_tri_e = np.zeros(nlevdecomp) # "b" vector for tridiagonal matrix
	c_tri_e = np.zeros(nlevdecomp) # "c" vector for tridiagonal matrix
	r_tri_e = np.zeros(nlevdecomp) #"r" vector for tridiagonal solution

	a_tri_dz = np.zeros(nlevdecomp) # "a" vector for tridiagonal matrix with considering the depth
	b_tri_dz = np.zeros(nlevdecomp) # "b" vector for tridiagonal matrix with considering the depth
	c_tri_dz = np.zeros(nlevdecomp) # "c" vector for tridiagonal matrix with considering the depth

	d_p1_zp1 = np.zeros(nlevdecomp+1) # diffusivity/delta_z for next j
	# (set to zero for no diffusion)
	d_m1_zm1 = np.zeros(nlevdecomp+1) # diffusivity/delta_z for previous j
	# (set to zero for no diffusion)
	f_p1 = np.zeros(nlevdecomp+1) # water flux for next j
	f_m1 = np.zeros(nlevdecomp+1) # water flux for previous j
	pe_p1 = np.zeros(nlevdecomp+1) # Peclet # for next j
	pe_m1 = np.zeros(nlevdecomp+1) # Peclet # for previous j
	...
	if  max(altmax, altmax_lastyear) <= max_altdepth_cryoturbation and max(altmax, altmax_lastyear) > 0.:
		...
		for j in np.arange(nlevdecomp+1):
			if j <= nbedrock:
...
def catanf(t1):
	catanf_results = 11.75 +(29.7 / np.pi) * np.arctan( np.pi * 0.031  * ( t1 - 15.4 ))
