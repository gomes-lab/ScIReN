...
def fun_bulk_simu(tensor_para, tensor_frocing_steady_state, tensor_obs_layer_depth):
	...
	bulk_A_doc = (torch.ones((profile_num, 1))*np.nan).to(device)
	bulk_E_mic = (torch.ones((profile_num, 1))*np.nan).to(device)
	bulk_A_mic = (torch.ones((profile_num, 1))*np.nan).to(device)
	bulk_A_POM = (torch.ones((profile_num, 1))*np.nan).to(device)
	bulk_A_MAOM = (torch.ones((profile_num, 1))*np.nan).to(device)
	...
	for iprofile in range(0, profile_num):
		...
		if torch.isnan(torch.sum(profile_para)) == False and \
			...
			bulk_A_DOC_profile, bulk_E_mic_profile, bulk_A_mic_profile, bulk_A_POM_profile, bulk_A_MAOM_profile, \
			w_scaler_profile, bulk_K_profile, bulk_V_profile, bulk_xi_profile, bulk_I_profile, \
			litter_fraction_profile	  = matrix_fun_COMPAS(profile_para, profile_force_steady_state)
			...
			bulk_A_doc[iprofile, :] = bulk_A_DOC_profile
			bulk_E_mic[iprofile, :] = bulk_E_mic_profile
			bulk_A_mic[iprofile, :] = bulk_A_mic_profile
			bulk_A_POM[iprofile, :] = bulk_A_POM_profile
			bulk_A_MAOM[iprofile, :] = bulk_A_MAOM_profile
	...
	return carbon_input, cpool_steady_state, cpools_layer, soc_layer, total_res_time, total_res_time_base, res_time_base_pools, t_scaler, bulk_A_doc, bulk_E_mic, bulk_A_mic, bulk_A_POM, bulk_A_MAOM, w_scaler, bulk_K, bulk_V, bulk_xi, bulk_I, litter_fraction
...
def matrix_fun_COMPAS(tensor_para, tensor_forcing_steady_state):
	...
	B_matrix[40:60, 0] = ((input_tot_litter2 + input_tot_litter3) * vertical_input) / (carbon_input_sum * dz[0:n_soil_layer])
	B_matrix[60:140, 0] = 0
	...
	total_res_time = torch.cat((residence_time[60:80, :], residence_time[80:100, :], residence_time[100:120, :], residence_time[120:140, :]), dim = 1)
	total_res_time = torch.sum(total_res_time, axis = 1)

	total_res_time_base = torch.cat((residence_time_baseline[60:80, :], residence_time_baseline[80:100, :], residence_time_baseline[100:120, :], residence_time_baseline[120:140, :]), dim = 1)
	...
	decom_cpools = torch.tensor([tau4cwd, tau4l1, tau4l2, tau4doc, tau4mic, tau4poc, tau4maom]).reciprocal().unsqueeze(1).to(device)  # , dtype=torch.float32
	...
	total_decom_litter = torch.stack([torch.sum(decom_cpool_apparent[20:40, :] * dz[0:n_soil_layer], dim = 0),
		torch.sum(decom_cpool_apparent[40:60, :] * dz[0:n_soil_layer], dim = 0)], dim = 0).to(device)
	...
	litter_resp_vector = torch.stack((resp_vector[20:40], resp_vector[40:60]), dim = 0).t().to(device)
	...
	for idoner in range(len(cpool_flow_rate)):
		total_donor_flow[idoner] = torch.sum(torch.matmul(donor_pool_size[:, idoner], donor_decomp[idoner]) * bulk_xi_layer * dz[:n_soil_layer])
	bulk_A_doc = torch.sum(cpool_flow_rate * total_donor_flow) / torch.sum(total_donor_flow)
	...
	for idoner in range(len(cpool_flow_rate)):
		total_donor_flow[idoner] = torch.sum(torch.matmul(donor_pool_size[:, idoner], donor_decomp[idoner]) * bulk_xi_layer * dz[:n_soil_layer])
	bulk_E_mic = torch.sum(cpool_flow_rate * total_donor_flow) / torch.sum(total_donor_flow)
	# carbon flow with CUE
	cue_cpool = torch.tensor([e52, e53, e54]).to(device) # e52, e53, e54
	...
	for idoner in range(len(cpool_flow_rate)):
		total_donor_flow[idoner] = torch.sum(torch.matmul(donor_pool_size[:, idoner], donor_decomp[idoner]) * bulk_xi_layer * dz[:n_soil_layer])
	bulk_A_MAOM = torch.sum(cpool_flow_rate * total_donor_flow) / torch.sum(total_donor_flow)
	...
	return carbon_input_sum, cpool_steady_state, cpools_layer, soc_layer, \
		total_res_time, total_res_time_base, res_time_base_pools, \
		t_scaler, bulk_A_doc, bulk_E_mic, bulk_A_mic, bulk_A_POM, bulk_A_MAOM, \
		w_scaler, bulk_K, bulk_V, bulk_xi, bulk_I, litter_fraction
