...
class nn_model(nn.Module):
	def __init__(self, var_idx_to_emb):
		...
		# fifth layer
		self.l5 = nn.Linear(256, 20) # 20 layers
	...
	def forward(self, input_var, wosis_depth):
		...
		h4 = self.dropout(h4)
		h5 = torch.relu(self.l5(h4)) # hardtanh(self.l5(h4))
		...
		simu_soc = select_depth(h5, forcing, obs_depth)
...
def worker(rank, world_size):
	...
	for iepoch in range(num_epoch):
		...
		# If runtimes are over 11.75 hours, save checkpoint and exit
		whether_checkpoint = False
		if time.time() - start_time > 42300:
			if rank == 0:
	...
	if rank == 0:
		with torch.no_grad():
			best_guess_val_y_hat = best_guess_model(val_x, val_z)
			best_guess_train_y_hat = best_guess_model(train_x, train_z)
			if test_split_ratio != 0:
				best_guess_test_y_hat = best_guess_model(test_x, test_z)
			...
			if np.isnan(binn_obs_soc[i, :]).all() or np.isnan(best_simu_soc[i, :]).all():
				...
				for j in range(len(simu_soc)):
					if np.isnan(obs_soc[j]) or np.isnan(simu_soc[j]):
						...
						temp_simu_sum += simu_soc[j]
						temp_obs_sum += obs_soc[j]
				scaled_diff[i] = np.abs(temp_simu_sum - temp_obs_sum)/temp_obs_sum
		...
		# save test_lons, test_lats, scaled_diff into csv
		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/test_lons_' + job_id + '.csv', test_lons, delimiter = ',')
		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/test_lats_' + job_id + '.csv', test_lats, delimiter = ',')
		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/test_scaled_diff_' + job_id + '.csv', scaled_diff, delimiter = ',')
			...
			if np.isnan(val_simu_soc[i, :]).all() or np.isnan(binn_obs_soc[i, :]).all():
				...
				for j in range(len(simu_soc)):
					if np.isnan(obs_soc[j]) or np.isnan(simu_soc[j]):
						...
						# Calculate the scaled difference
						temp_simu += simu_soc[j]
						temp_obs_sum += obs_soc[j]
				scaled_diff[i] = np.abs(temp_simu - temp_obs_sum)/temp_obs_sum

		# Plot the scaled difference
		visualization_utils.plot_observations_world_map(val_lons, val_lats, scaled_diff, PLOT_DIR, "validation_scaled_diff_" + job_id)
		# save val_lons, val_lats, scaled_diff into csv
		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/val_lons_' + job_id + '.csv', val_lons, delimiter = ',')
		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/val_lats_' + job_id + '.csv', val_lats, delimiter = ',')
		np.savetxt(data_dir_output + 'neural_network/' + job_id + '/val_scaled_diff_' + job_id + '.csv', scaled_diff, delimiter = ',')
