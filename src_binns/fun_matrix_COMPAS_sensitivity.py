...
def fun_model_sensitivity(tensor_para, tensor_frocing_steady_state):
	...
	for iprofile in range(0, profile_num):
		...
		if torch.isnan(torch.sum(profile_para)) == False and \
			...
			# print(profile_para)
			# model simulation
			profile_simu_soc, profile_simu_POM, profile_simu_MAOM, whether_singular_profile = matrix_fun_COMPAS(profile_para, profile_force_steady_state)
			
			# save simulation results
			if whether_singular_profile == False:
				simu_ouput_soc[iprofile, 0:20] = profile_simu_soc * dz[0:20] 
				simu_output_POM[iprofile, 0:20] = profile_simu_POM * dz[0:20]
				simu_output_MAOM[iprofile, 0:20] = profile_simu_MAOM * dz[0:20]
			else:
				simu_ouput_soc[iprofile, 0:20] = torch.nan
				simu_output_POM[iprofile, 0:20] = torch.nan
				simu_output_MAOM[iprofile, 0:20] = torch.nan
...
def matrix_fun_COMPAS(tensor_para, tensor_forcing_steady_state):
	...
	whether_singular_profile = False
	...
	try:
		...
		traceback.print_exc()
		whether_singular_profile = True
		cpool_steady_state = torch.zeros([npool_vr, npool_vr]).to(device)
		pass
	...
	# outcome = soc_layer
	return soc_layer, POM_layer, MAOM_layer, whether_singular_profile
