...
# Runs Blackbox-Hybrid training on real labels. Usage on Slurm:
...
#SBATCH -J 4b_real_blackboxhybrid
# Request 4 GPUs 
# #SBATCH --gpus 4
            ...
            do
                ...
                do
                    ...
                    python3 binns_DDP.py --data_seed 12345 --representative_sample --split grid2 --cross_val_idx $FOLD --n_folds 5 \
                        ...
                        --num_CPU 8 --use_ddp 1 --job_scheduler slurm --time_limit 11.5 --note "4B_REAL_BLACKBOXHYBRID" $PLOT_STR
                done
            done
        done
    done
...
#                         --features ten --labels real \
#                         --losses smooth_l1 param_reg --lambdas 1 $PREG --param_constraint sigmoid \
#                         --num_CPU 8 --use_ddp 1 --job_scheduler slurm --time_limit 11.5 --note "REAL_BINN" $PLOT_STR
#                 done
#             done
#         done
#     done
