...
# Commenting out these dependencies as we are not using GNN currently, only the spatial positional encoder
...
def get_activation_function(activation, context_str):
    if activation == "leakyrelu":
        ...
        return nn.Tanh()
    elif activation == 'none':  # @NOTE: added option for no activation function
...
class MultiLayerFeedForwardNN(nn.Module):
    ...
    def __init__(self, input_dim,
        ...
        if self.num_hidden_layers <= 0:
            ...
            self.layers.append( SingleFeedForwardNN(input_dim = self.hidden_dim,
                                                    output_dim = self.output_dim,
                                                    dropout_rate = None,  # self.dropout_rate,  # @NOTE: documentation claims last layer has no dropout/activation, but the code had them. Updated code to match documentation.
...
class GridCellSpatialRelationEncoder(nn.Module):
    ...
    def forward(self, coords):
        ...
        spr_embeds = torch.tensor(spr_embeds, dtype=torch.float32).to(device)  # torch.FloatTensor(spr_embeds).to(device)  @NOTE modified
        if self.ffn is not None:
